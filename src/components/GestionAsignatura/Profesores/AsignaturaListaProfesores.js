				...
				tbodyListaProfesores[i] = (
					<ContractData	key={i}
									...
									render={(profesor) => (
										<tr>
											<td>
												<Link to={`/gestion-profesores/profesor/${profesor.addrEthProf}`}>{shortenEthAddress(profesor.addrEthProf)}</Link>
												...
												</button>
												{
													...
													?
													<button type="button" className="btn btn-outline-danger btn-delete" onClick={() => this.eliminarAlumno(addrEthProf)}>
		...
		if (hayAlgunProfesor) {
			return (
				<>
					...
					<p>{this.props.profesoresLength} profesoresLength || {this.props.numProfesores} profesores</p>
